...
- **LLM**: Ollama (컨테이너), 기본 모델 `gemma4:e4b` (한국어 품질 기준으로 선정)
- **Embedding**: Ollama `bge-m3` (의미적 중복 질문 감지용, 다국어 지원)
...
└── game/                        # 메인 앱
    ...
    ├── twenty_questions_logic.py  # 프롬프트 구성 + 턴 진행 로직
...
넓은 범주 → 구체적 특징 순서로 질문하도록 시스템 프롬프트에서 유도. 카테고리별 질문 로드맵(`CATEGORY_ROADMAPS`)으로 축 우선순위를 강제하며, 인물 카테고리는 실존/가상 여부에 따라 분기한다.

### 6. Best-of-N 질문 선택
매 턴 `BEST_OF_N`(기본 3)개의 질문 후보를 병렬 생성(`ThreadPoolExecutor`)하고, 별도 '선택기' LLM 호출이 현재 남은 후보군 기준으로 변별력이 가장 높은(50%에 가장 가까운) 질문을 선택한다. 생성은 `temperature=0.6`(다양성), 선택은 `0.1`(결정론).

### 7. 후보군 동적 추정
매 턴 별도 LLM 호출로 남은 대상 후보 Top-N(기본 10)을 추정해 `GameSession.candidates`에 저장. 질문 생성 시 이 후보군을 프롬프트에 포함해 정보 이득 높은 질문을 유도한다.

### 8. 임베딩 기반 중복 질문 감지
`bge-m3`로 각 질문의 임베딩을 계산해 `QA.question_embedding`에 저장. 새 질문이 과거 질문들과 코사인 유사도 ≥ `SIMILARITY_THRESHOLD`(기본 0.92)이면 의미적 중복으로 간주하고 재생성. Best-of-N 경로와 단일 경로 양쪽에서 적용.

### 9. 금지 표현 필터링
정보 이득이 낮은 표현("특정", "관련 있" 등)과 복수/모호 추측 표현("또는", "중 한", "," 등)을 감지해 최대 `MAX_REGENERATE`(기본 2)회 재생성한다. 재시도 시 이전 거부 사유를 프롬프트에 명시해 같은 실수 반복을 방지.
...
- `pending_question` (TextField, default="") — LLM이 제시했고 아직 답이 오지 않은 질문. 답이 오면 QA로 옮기고 비움.
- `candidates` (JSONField, default=list) — 매 턴 LLM이 추정한 남은 후보 Top-N 목록.
- `created_at` (auto)
...
- `question_embedding` (JSONField, null=True) — bge-m3 임베딩. 실패 시 null.
- `created_at` (auto)
- `Meta.ordering = ['turn']`, `UniqueConstraint(session, turn)`
...
### 서비스
- **web**: Django 앱, 포트 `8001:8000` (호스트 8000은 다른 프로젝트가 선점 중이라 8001로 매핑), 코드 볼륨 마운트로 핫리로드
...
- 프롬프트 문자열은 `twenty_questions_logic.py` 상단 상수로 분리
...
6. **Ollama 클라이언트 + 로직 작성**
   ...
   - `game/twenty_questions_logic.py`: 시스템 프롬프트 상수, `next_turn(session)` 함수
   - JSON 파싱 방어 로직 포함
...
9. **동작 테스트**
   - `docker compose up -d`
   - `docker compose exec ollama ollama pull gemma4:e4b` (최초 1회)
   - `docker compose exec ollama ollama pull bge-m3` (임베딩 모델, 최초 1회)
...
- **한국어 품질**: 현재 `gemma4:e4b`가 `qwen2.5:7b`/`llama3.1:8b` 대비 한국어 품질이 우수하다고 판단되어 기본 모델로 사용 중. 교체 제안 전에 반드시 직접 비교 후 결정.
- **포트 충돌 (11434)**: 호스트의 기존 Ollama와 겹침. compose에서 `11435:11434`로 매핑되어 있는지 확인.
- **포트 충돌 (8000)**: 호스트 8000 포트는 다른 프로젝트가 선점 → web은 `8001:8000`으로 매핑. `http://localhost:8001`로 접속.
- **임베딩 누락으로 중복 감지 실패**: `bge-m3` 모델이 pull되어 있는지 확인 (`docker compose exec ollama ollama list`).